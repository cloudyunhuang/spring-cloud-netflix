...
public class EurekaController {
    ...
    public String detail(HttpServletRequest request, @RequestParam(required = false) String id, Map<String, Object> model) {
...
//                instanceInfoMap.put(id, info);
//                String url = info.getStatusPageUrl();
                String dashboardPathCopy = dashboardPath;
                if (!dashboardPathCopy.endsWith("/")) {
                    dashboardPathCopy = dashboardPathCopy + "/";
                }
                if (!dashboardPathCopy.startsWith("/")) {
                    dashboardPathCopy = "/" + dashboardPathCopy;
                }
                String url = "." + dashboardPathCopy + "detail?id=" + id;
...
//                    String id = p.first();
//                    InstanceInfo instanceInfo = instanceInfoMap.get(id);
//                    if (instanceInfo != null && instanceInfo.getMetadata() != null && !instanceInfo.getMetadata().isEmpty()) {
//                        StringBuilder title = new StringBuilder();
//                        String contextPath = instanceInfo.getMetadata().get(CONTEXT_PATH_KEY);
//                        if (!StringUtils.isEmpty(contextPath)) {
//                            title.append("Context Path [" + contextPath + "]");
//                        }
//
//                        for (Map.Entry<String, String> entry1 : instanceInfo.getMetadata().entrySet()) {
//                            if (!CONTEXT_PATH_KEY.equals(entry1.getKey())) {
//                                title.append("[" + entry1.getValue() + ":" + entry1.getKey() + "] ");
//                            }
//
//                        }
